...
def main():
    ...
    if st.sidebar.button("📜Generate Report📜"):
        ...
        # Grading_report = [{'role': 'system', 'content': open_file(Comp_loc)}, {'role': 'user', 'content': st.session_state.get('Guide-Assessment', '')}]
        # report, total_tokens = chatbotGPT4(Grading_report)

        st.sidebar.download_button(
            label="Download Report",
            data=bytes(Grading_combined, encoding='utf-8'),  # Convert string to bytes
